...
class SequentialDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train' and 'neg_samp' in configs['data'] and configs['data']['neg_samp']:
            return self.uids[idx], torch.LongTensor(self._pad_seq(seq_i)), self.last_items[idx], self.negs[idx],self.gender[idx]
        else:
            return self.uids[idx], torch.LongTensor(self._pad_seq(seq_i)), self.last_items[idx],self.gender[idx]
